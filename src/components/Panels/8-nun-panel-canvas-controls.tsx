...
function PanelCanvasControlsInternals() {
    return (
        <div className="flex flex-col">
            ...
            <div className="flex flex-col">
                ...
                <div className="mt-2 space-y-1.5">
                    ...
                    <Checkbox label="Show controls point" tooltip="Show controls point" atom={showCPsAtom} />
...
function ButtonZoom({ label, atom, value, className = '' }: { label: string; atom: WritableAtom<null, number>; value: number; className?: string; }) {
    ...
    return (
        <button className={classNames(`px-1 pb-px h-6 text-xs text-slate-400 border-slate-500 bg-slate-800 shadow-sm shadow-slate-800 active:scale-[.97] select-none`, className)} onClick={() => setIsDown(value)}
        >
            {label}
        </button>
    );
